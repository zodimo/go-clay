...
func TestNewMemArray(t *testing.T) {
	...
	t.Run("panic when creating array with zero capacity", func(t *testing.T) {
		func() {
			defer func() {
				if r := recover(); r == nil {
					t.Error("expected panic when creating array with zero capacity")
				}
			}()
			NewMemArray[int](0)
		}()
...
func TestMemArray_Get(t *testing.T) {
	...
	t.Run("returns nil for index >= array length", func(t *testing.T) {
		arr := NewMemArray[int](5)

		// MemArray_Get checks against Length()
		panicked := false
		func() {
			defer func() {
				if r := recover(); r != nil {
		...
		if !panicked {
			t.Error("expected panic when accessing index >= length")
		}
...
func TestMArray_GetValue(t *testing.T) {
	...
	t.Run("panic on get value for index >= length", func(t *testing.T) {
		arr := NewMemArray[int](5)
		...
		func() {
			...
			_ = MArray_GetValue(&arr, 10) // This should panic
		}()
		if !panicked {
			t.Error("expected panic when accessing index >= length")
		}
...
func TestMArray_Set(t *testing.T) {
	t.Run("sets value at valid index < length", func(t *testing.T) {
		arr := NewMemArray[int](5)
		MArray_Add(&arr, 10)
		MArray_Add(&arr, 20)
		MArray_Add(&arr, 30)
	...
	t.Run("sets value at last valid index", func(t *testing.T) {
		...
		MArray_Set(&arr, 2, 99)
		if MArray_GetValue(&arr, 2) != 99 {
			t.Errorf("expected InternalArray[2] = 99, got %d", MArray_GetValue(&arr, 2))
		}
	})

	t.Run("panic on set value for negative index", func(t *testing.T) {
		arr := NewMemArray[int](5)
		...
		if !panicked {
			t.Error("expected panic when setting value for negative index")
		}
	})

	t.Run("panic on set value for index >= length", func(t *testing.T) {
		arr := NewMemArray[int](5)
		MArray_Add(&arr, 20)
		MArray_Add(&arr, 30)
		...
		if !panicked {
			t.Error("expected panic when setting value for index >= length")
		}
	})
...
func TestMArray_RemoveSwapback(t *testing.T) {
	...
	t.Run("removes from single element array", func(t *testing.T) {
		arr := NewMemArray[int](4) // @todo should be 1
		MArray_Add(&arr, 42)
	...
	t.Run("panic for negative index", func(t *testing.T) {
		arr := NewMemArray[int](5)
		MArray_Add(&arr, 10)
		MArray_Add(&arr, 20)
		...
		func() {
			...
			MArray_RemoveSwapback(&arr, -1)
		}()
		if !panicked {
			t.Error("expected panic when removing from negative index")
		}
	})

	t.Run("panic for index >= length", func(t *testing.T) {
		arr := NewMemArray[int](5)
		MArray_Add(&arr, 10)
		MArray_Add(&arr, 20)
		...
		func() {
			...
			MArray_RemoveSwapback(&arr, 2)
		}()
		if !panicked {
			t.Error("expected panic when removing from index >= length")
		}
	})
...
func TestMemArray_WithArena(t *testing.T) {
	...
	t.Run("MArray_Get with arena", func(t *testing.T) {
		arena := NewArenaWithSizeUnsafe(2048)
		arr := NewMemArray[int](5, MemArrayWithArena[int](arena))
		MArray_Add(&arr, 42)
		MArray_Add(&arr, 10)
		MArray_Add(&arr, 20)
		...
		ptr1 := MArray_GetValue(&arr, 1)
		ptr2 := MArray_GetValue(&arr, 2)
		if ptr1 != 10 {
			t.Errorf("expected ptr1 = 10, got %v", ptr1)
		}
		if ptr2 != 20 {
			t.Errorf("expected ptr2 = 20, got %v", ptr2)
		}
		...
		if !panicked {
			t.Error("expected panic when accessing index < 0")
		}
	})
	...
	t.Run("full workflow with arena", func(t *testing.T) {
		arena := NewArenaWithSizeUnsafe(2048)
		arr := NewMemArray[int](10, MemArrayWithArena[int](arena))
	...
	t.Run("capacity limit with arena", func(t *testing.T) {
		arena := NewArenaWithSizeUnsafe(2048)
		arr := NewMemArray[int](2, MemArrayWithArena[int](arena))
		...
		// This should panic because Length (2) == Capacity (2)
		panicked := false
		func() {
			defer func() {
				if r := recover(); r != nil {
		...
		if !panicked {
			t.Error("expected panic when capacity reached")
		}
		if arr.Length() != 2 {
			t.Errorf("expected Length = 2, got %d", arr.Length())
		}
