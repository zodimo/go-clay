		...
		case clay.CommandRectangle:
			if rectCmd, ok := cmd.Data.(clay.RectangleCommand); ok {
				...
				return NewRenderError(
					...
					"Render",
					fmt.Sprintf("invalid rectangle command data for element %d", cmd.ID),
				)
			}
		case clay.CommandText:
			if textCmd, ok := cmd.Data.(clay.TextCommand); ok {
				...
				return NewRenderError(
					ErrorTypeInvalidData,
					"Render",
					fmt.Sprintf("invalid text command data for element %d", cmd.ID),
				)
			}
		case clay.CommandImage:
			if imageCmd, ok := cmd.Data.(clay.ImageCommand); ok {
				if err := r.renderImageWithBounds(bounds, imageCmd); err != nil {
					return err
				}
			} else {
				return NewRenderError(
					ErrorTypeInvalidData,
					"Render",
					fmt.Sprintf("invalid image command data for element %d", cmd.ID),
				)
			}
		case clay.CommandBorder:
			if borderCmd, ok := cmd.Data.(clay.BorderCommand); ok {
				if err := r.renderBorderWithBounds(bounds, borderCmd); err != nil {
					return err
				}
			} else {
				return NewRenderError(
					ErrorTypeInvalidData,
					"Render",
					fmt.Sprintf("invalid border command data for element %d", cmd.ID),
				)
			}
		case clay.CommandClipStart:
			if clipCmd, ok := cmd.Data.(clay.ClipStartCommand); ok {
				if err := r.renderClipStartWithBounds(bounds, clipCmd); err != nil {
					return err
				}
			} else {
				return NewRenderError(
					ErrorTypeInvalidData,
					"Render",
					fmt.Sprintf("invalid clip start command data for element %d", cmd.ID),
				)
			}
		case clay.CommandClipEnd:
			if clipCmd, ok := cmd.Data.(clay.ClipEndCommand); ok {
				if err := r.renderClipEndWithBounds(bounds, clipCmd); err != nil {
					return err
				}
			} else {
				return NewRenderError(
					ErrorTypeInvalidData,
					"Render",
					fmt.Sprintf("invalid clip end command data for element %d", cmd.ID),
				)
			}
		case clay.CommandCustom:
			if customCmd, ok := cmd.Data.(clay.CustomCommand); ok {
				if err := r.renderCustomWithBounds(bounds, customCmd); err != nil {
					return err
				}
			} else {
				return NewRenderError(
					ErrorTypeInvalidData,
					"Render",
					fmt.Sprintf("invalid custom command data for element %d", cmd.ID),
				)
			}
		default:
			return NewRenderError(
...
func (r *GioRenderer) renderRectangleWithBounds(bounds clay.BoundingBox, cmd clay.RectangleCommand) error {
	// Validate bounds
	...
	if r.isCornerRadiusZero(cmd.CornerRadius) {
...
func (r *GioRenderer) renderSimpleRectangle(bounds clay.BoundingBox, cmd clay.RectangleCommand) error {
...
func (r *GioRenderer) renderRoundedRectangle(bounds clay.BoundingBox, cmd clay.RectangleCommand) error {
...
func (r *GioRenderer) renderTextWithBounds(bounds clay.BoundingBox, cmd clay.TextCommand) error {
	// Validate bounds
	if bounds.Width <= 0 || bounds.Height <= 0 {
		return NewRenderError(
			ErrorTypeInvalidInput,
	...
	label := widget.Label{
		Alignment:  r.mapClayTextAlignment(cmd.Alignment),
	...
	label.Layout(gtx, shaper, fontObj, unit.Sp(cmd.FontSize), cmd.Text, colorCallOp)
...
func (r *GioRenderer) mapClayTextAlignment(clayAlign clay.TextAlignment) text.Alignment {
	switch clayAlign {
	case clay.TextAlignLeft:
		return text.Start
	case clay.TextAlignCenter:
		return text.Middle
	case clay.TextAlignRight:
		return text.End
	default:
		return text.Start
	}
...
func (r *GioRenderer) renderImageWithBounds(bounds clay.BoundingBox, cmd clay.ImageCommand) error {
	// Validate bounds
	if bounds.Width <= 0 || bounds.Height <= 0 {
		return NewRenderError(
			ErrorTypeInvalidInput,
	...
	// Validate image data
	imageData, ok := cmd.ImageData.([]byte)
	if !ok {
		return NewRenderError(
			ErrorTypeInvalidInput,
			"renderImageWithBounds",
...
func (r *GioRenderer) renderBorderWithBounds(bounds clay.BoundingBox, cmd clay.BorderCommand) error {
	...
	r.renderBorderSides(rect, cmd.Width, gioColor, cmd.CornerRadius)

	return nil
}

// renderClipStartWithBounds starts clipping using bounds from RenderCommand
func (r *GioRenderer) renderClipStartWithBounds(bounds clay.BoundingBox, cmd clay.ClipStartCommand) error {
...
func (r *GioRenderer) RenderRectangle(cmd clay.RectangleCommand) error {
	if r.ops == nil {
...
func (r *GioRenderer) RenderText(cmd clay.TextCommand) error {
	if r.ops == nil {
		return fmt.Errorf("operations context is nil")
	}
...
func (r *GioRenderer) RenderImage(cmd clay.ImageCommand) error {
	...
	cachedImage, err := r.cache.GetOrCreateImage(cmd.ImageData, filter)
	if err != nil {
	...
	tintColor := r.cache.GetOrCreateColor(cmd.TintColor.R, cmd.TintColor.G, cmd.TintColor.B, cmd.TintColor.A)
...
func (r *GioRenderer) RenderBorder(cmd clay.BorderCommand) error {
	defer func() {
	...
	// Convert Clay color to Gio color
	gioColor := r.cache.GetOrCreateColor(cmd.Color.R, cmd.Color.G, cmd.Color.B, cmd.Color.A)
	...
	r.renderBorderSides(bounds, cmd.Width, gioColor, cmd.CornerRadius)

	return nil
}

// validateBorderCommand validates the border command parameters
func (r *GioRenderer) validateBorderCommand(cmd clay.BorderCommand) error {
	// Validate color values
	if err := ValidateColor(cmd.Color.R, cmd.Color.G, cmd.Color.B, cmd.Color.A); err != nil {
		return err
	}

	// Validate border widths (must be non-negative)
	if cmd.Width.Left < 0 || cmd.Width.Right < 0 || cmd.Width.Top < 0 || cmd.Width.Bottom < 0 {
		return NewRenderError(
			ErrorTypeInvalidInput,
...
func (r *GioRenderer) validateImageCommand(cmd clay.ImageCommand) error {
	// Validate image data is not nil
	if cmd.ImageData == nil {
		return NewRenderError(
			ErrorTypeInvalidInput,
	...
	if err := ValidateColor(cmd.TintColor.R, cmd.TintColor.G, cmd.TintColor.B, cmd.TintColor.A); err != nil {
		return err
	}
...
func (r *GioRenderer) RenderClipStart(cmd clay.ClipStartCommand) error {
	defer func() {
...
func (r *GioRenderer) RenderCustom(cmd clay.CustomCommand) error {
	if r.customRegistry == nil {
		return NewRenderError(
			ErrorTypeInvalidInput,
			"RenderCustom",
			"Custom command registry not initialized",
		)
	}
...
func (r *GioRenderer) renderClipEndWithBounds(bounds clay.BoundingBox, cmd clay.ClipEndCommand) error {
	if len(r.clipStack) == 0 {
		return NewRenderError(
			ErrorTypeInvalidState,
...
func (r *GioRenderer) renderCustomWithBounds(bounds clay.BoundingBox, cmd clay.CustomCommand) error {
	if r.customRegistry == nil {
		return NewRenderError(
			ErrorTypeInvalidState,
